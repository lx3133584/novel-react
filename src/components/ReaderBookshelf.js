        ...
        {if (this.props.token) {
            ...
            ><Book
                    ...
                    detailList={[
                        ...
                        {
                            key: '阅读进度',
                            value: itemBox.progress ? `${itemBox.progress}/${item.countChapter} (${(itemBox.progress / item.countChapter).toFixed(2)}%)` : '未读',
                        },
                    ]} />
            </SwipeAction>})}
            {!this.props.list.length && <div style={{textAlign: 'center'}}><WhiteSpace size='xl'/>你的书架空空如也</div>}
        </div>} else {
            return <LoginBtn />
        }
        }
